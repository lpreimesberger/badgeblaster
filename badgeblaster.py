...
while 1:
    user_input = prompt('hOOMAN (r/w/q)> ',
    ...
    if user_input.startswith('w'):
        chopped = user_input.split(" ")
        if len(chopped) > 1:
            fake_name = chopped[1]
        else:
            fake_name = random.choice(ADJECTIVES).capitalize() + " " + random.choice(NOUNS).capitalize()
        print(f"using fake name {fake_name} - manual entry or abort (m/a)")
        user_input = prompt('manual or abort or continue (m/a/c)> ')
        if user_input == "a":
            continue
        if user_input == "m":
            fake_name = prompt("enter playa name> ")
        print(f"using playa name {fake_name}")
        ...
        print(f"refresh hooman {hooman_id}")
        tag.ndef.records = [ndef.TextRecord(f"xnm:{fake_name}"), ndef.TextRecord(f"xyr:2023"), ndef.TextRecord("xis:Pussy Avalanche"), ndef.TextRecord("xar:Toxoplasmosis")]
        clf.close()
    if user_input == 'r':
        print("SCAN HOOMAN")
        tag = clf.connect(rdwr={
            ...
            'beep-on-connect': True})
        hooman_id = binascii.hexlify(tag.identifier).decode('utf-8')
        print(f"base scan of hooman {hooman_id}")
